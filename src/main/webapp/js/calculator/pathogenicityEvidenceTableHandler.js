...
function addOtherModifiedTagValuesToBeMarkedAsUsed(loadedEvidenceSet){
    ...
    for(let g in loadedEvidenceSet){
        let evdTagVal = loadedEvidenceSet[g];
        var basicEvdcValue = evdTagVal.split("-")[0].trim(); //extracts, example BS1 from BS1 or BS1 - Supporting
                    ...
                    div.className="dropdownEvidenceCodes"
                        ...
                        if(validTags != null && validTags.length > 0){
                            ...
                            for(let g in validTags){
                                ...
                                let tagIsUsed = isThisEvidenceUsed(evidenceTagName, loadedEvidenceSet);
                                let tadToBeDiasabled = false;
                                ...
                                if(tagIsUsed){
                                    ...
                                    tadToBeDiasabled = shouldThisEvidenceBeDisabled(evidenceTagName, evidenceTagsToBeDisabled);
                                }

                                if(tagIsUsed || tadToBeDiasabled){
                                    li.style.color = "red";                            
                                }else{
...
function isThisEvidenceUsed(evidenceTag, loadedEvidenceTags){
    if(loadedEvidenceTags == null || loadedEvidenceTags.length == 0){
        return false;
    }
    for(let i in loadedEvidenceTags){
        if(loadedEvidenceTags[i] == evidenceTag){
            return true;
        }
    }
    return false;
